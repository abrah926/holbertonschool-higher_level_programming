...
# 3.Serving JSON Data / Import the jsonify function from Flask
...
# 2.Creating Your First Endpoint / Visit http://localhost:5000 in your browser or use curl to see the message.
PORT = 5000
...
# decorator, followed by a function that returns the desired response for that route.
@app.route("/")
def home():  # 2.Creating Your First Endpoint / Define a route for the root URL (“/”) and create a function
    return "Welcome to the Flask API!"


# 3.Serving JSON Data / Create a new route /data and associate a function with it
@app.route('/data', methods=['GET'])
def get_usernames():
    return jsonify(list(users.keys()))


# 4.Expanding Your API / Add a few more endpoint /status: It should return OK.
@app.route('/status', methods=['GET'])
def status():
    return "OK"


# 4.Expanding Your API / Add a few more endpoint //users/<username>: Returns the full object corresponding to the provided username. (Hint: Use Flask’s dynamic route feature)
